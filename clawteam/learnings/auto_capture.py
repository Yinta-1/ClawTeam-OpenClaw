...
class AutoCaptureEngine:
    ...
    def __init__(self, learnings_dir: Optional[str] = None):
        ...
        """
        self.learnings_dir = Path(learnings_dir or "~/.openclaw/workspace/.learnings").expanduser()
    ...
    def evaluate_task_result(
        ...
        if status == "failed" and error:
            ...
            entry = ExperienceEntry(
                entry_type=LearningType.BEST_PRACTICE,
                summary=f"节省 {task_result.get('time_saved_minutes')} 分钟的高效方法",
                details=f"任务ID: {task_id}\n方法描述: {task_result.get('method', '未知方法')}",
                category="best_practice",
                area=self._detect_area(task_result),
                priority="medium",
                session_ids=[session_id] if session_id else [],
    ...
    def _detect_area(self, task_result: Dict[str, Any]) -> str:
        ...
        infra_tools = ["docker", "kubernetes", "deploy", "infra", "ci/cd", "aws", "azure", "gcp"]
        ...
        for tool in tools:
            ...
            if any(ft in tool_lower for ft in frontend_tools):
                return "frontend"
            elif any(bt in tool_lower for bt in backend_tools):
                return "backend"
            elif any(it in tool_lower for it in infra_tools):
                return "infra"
            elif any(tt in tool_lower for tt in test_tools):
                return "tests"
            elif any(dt in tool_lower for dt in docs_tools):
                return "docs"
        ...
        if any(word in description for word in ["frontend", "ui", "界面", "网页"]):
            return "frontend"
        elif any(word in description for word in ["backend", "api", "数据库", "服务器"]):
            return "backend"
        elif any(word in description for word in ["部署", "运维", "docker", "kubernetes"]):
            return "infra"
        elif any(word in description for word in ["测试", "qa", "test", "测试用例"]):
            return "tests"
    ...
    def check_for_promotion(self, min_occurrences: int = 3, min_confidence: float = 0.8) -> List[ExperienceEntry]:
        ...
        for entry in self._experiences.values():
            if entry.count >= min_occurrences and not entry.resolved:
                # 检查是否已经值得晋升
                if entry.entry_type == LearningType.BEST_PRACTICE:
                    # 最佳实践更容易晋升
...
### 解决方案/最佳实践
{entry.resolution if entry.resolution else "待添加"}
...
"""
            ...
            lines.append("统计概览:")
            for entry_type, count in sorted(stats.items()):
                lines.append(f"  {entry_type}: {count} 条")
            lines.append("")
            lines.append("重点经验:")
