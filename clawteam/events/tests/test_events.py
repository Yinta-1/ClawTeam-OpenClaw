...
class TestEventModels(unittest.TestCase):
    ...
    def test_event_type_values(self):
        ...
        expected_types = [
            "team_created", "team_destroyed", "member_joined", "member_left", "member_alive",
            "task_created", "task_status_changed", "task_assigned", "task_completed", "task_blocked",
            "agent_spawned", "agent_terminated", "agent_idle", "agent_active", "turn_complete",
            "session_started", "session_ended",
            "message_sent", "message_received", "inbox_notification",
            "alert_triggered", "alert_acknowledged", "alert_resolved",
            "usage_recorded", "command_executed", "error_occurred",
        ]
...
class TestEventTracker(unittest.TestCase):
    ...
    def test_query_with_limit_offset(self):
        ...
        # Create 10 events
        events = [
            create_team_event(EventType.TEAM_CREATED, team_name=f"team-{i}")
            for i in range(10)
        ]
        self.tracker.track_batch(events)
    ...
    def test_thread_safety(self):
        ...
        events = []
        for i in range(50):
            events.append(create_team_event(
                EventType.TEAM_CREATED,
...
class TestEventIntegration(unittest.TestCase):
    ...
    def test_full_task_lifecycle(self):
        ...
        # Create task
        self.tracker.track(create_task_event(
