...
description: >
  ...
  to validate the full AgentTeam lifecycle. Runs a complete end-to-end test: cleanup →
  create team → create tasks with dependencies → spawn agents → wait for completion →
  verify results → cleanup.
version: 0.1.0
---

# AgentTeam End-to-End Test

This skill runs a full lifecycle test of AgentTeam: cleanup residual state, create a team
...
- AgentTeam installed (`pip install -e .` from the AgentTeam repo)
- `tmux` available
- `claude` CLI available
- Current directory is the AgentTeam git repo (for worktree isolation)
...
Remove **all** residual agentteam teams, worktrees, tmux sessions, and branches from any prior runs.
...
# 2. Remove ALL agentteam worktrees
for wt in $(git worktree list --porcelain | grep 'worktree.*/\.agentteam/' | awk '{print $2}'); do
...
rm -rf ~/.agentteam/teams/ ~/.agentteam/tasks/ ~/.agentteam/workspaces/ ~/.agentteam/inboxes/ ~/.agentteam/events/ ~/.agentteam/plans/
echo "data cleaned"
...
**Expected**: Only the main worktree remains, no agentteam branches, no agentteam tmux sessions,
`~/.agentteam/` contains only `config.json`.
...
```bash
agentteam team spawn-team e2e-test -d "End-to-end test team" -n leader
```
...
```bash
T1=$(agentteam --json task create e2e-test "Implement feature A" -o worker1 -d "Add a hello() function to a new file hello.py" | python3 -c "import sys,json;print(json.load(sys.stdin)['id'])")
T2=$(agentteam --json task create e2e-test "Implement feature B" -o worker2 -d "Add a goodbye() function to a new file goodbye.py" | python3 -c "import sys,json;print(json.load(sys.stdin)['id'])")
T3=$(agentteam --json task create e2e-test "Write tests" -o worker3 -d "Write pytest tests for hello.py and goodbye.py" --blocked-by "$T1,$T2" | python3 -c "import sys,json;print(json.load(sys.stdin)['id'])")
...
```bash
agentteam spawn --team e2e-test --agent-name worker1 \
...
agentteam spawn --team e2e-test --agent-name worker2 \
...
agentteam spawn --team e2e-test --agent-name worker3 \
...
- 3 worktrees under `~/.agentteam/workspaces/e2e-test/`
- 3 tmux windows
...
```bash
agentteam task wait e2e-test --timeout 600 --poll-interval 5
```
...
agentteam inbox receive e2e-test
```
...
# Remove ALL agentteam worktrees
for wt in $(git worktree list --porcelain | grep 'worktree.*/\.agentteam/' | awk '{print $2}'); do
  git worktree remove --force "$wt" 2>/dev/null
done
...
Replace `agentteam spawn` with `agentteam spawn subprocess claude` to test the subprocess backend instead of tmux. Note: `board attach` and tmux verification steps won't apply.
