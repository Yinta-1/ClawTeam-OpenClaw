...
| Los agentes generados no encuentran `agentteam` | Los agentes se ejecutan en shells nuevos sin PATH de pip | Verifica que el enlace simbólico `~/bin/clawteam` exista y que `~/bin` esté en PATH |
...
|---------|---------|---------|
| Directorio de datos | `CLAWTEAM_DATA_DIR` | `~/.agentteam` |
