...
**新增依赖**: `redis` (pypi)，可选安装 `pip install agentteam[redis]`
...
| 数据迁移工具 | `agentteam migrate file-to-redis` | 一起 |
...
### v0.3 已完成内容
- Config 系统：`agentteam config show/set/get/health`
- 多用户协作：`CLAWTEAM_USER` / `agentteam config set user`，(user, name) 复合唯一性
- Web UI：`agentteam board serve`，SSE 实时推送，深色主题看板
