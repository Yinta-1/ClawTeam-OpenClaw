...
Os agentes de IA atuais são poderosos, mas trabalham de forma **isolada**. O AgentTeam permite que os agentes se auto-organizem em equipes — dividindo trabalho, comunicando-se e convergindo em resultados sem microgerenciamento humano.
...
O líder chama `agentteam spawn` para criar workers. Cada um recebe sua própria **git worktree**, **janela tmux** e **identidade**.
...
Instale o AgentTeam e depois dê o prompt ao seu agente:

```
"Build a web app. Use agentteam to split the work across multiple agents."
```

O agente cria automaticamente uma equipe, gera workers, atribui tarefas e coordena — tudo via CLI `agentteam`.
...
|-------|--------------|--------|
| [OpenClaw](https://openclaw.ai) | `agentteam spawn tmux openclaw --team ...` | **Padrão** |
...
O AgentTeam requer **Python 3.10+**, **tmux** e pelo menos um agente de codificação CLI (OpenClaw, Claude Code, Codex, etc.).
...
> **⚠️ NÃO execute `pip install agentteam` ou `npm install -g agentteam` diretamente:**
> - `pip install agentteam` instala a versão upstream do PyPI, que usa `claude` como padrão e não possui adaptações OpenClaw.
> - `npm install -g agentteam` instala um pacote usurpador sem relação (publicado por `a9logic`). Se `agentteam --version` mostrar "Coming Soon", é o pacote errado. Execute primeiro `npm uninstall -g agentteam`.
>
...
pip install -e .    # ← Obrigatório! Instala do repositório local, NÃO é o mesmo que pip install agentteam
```
...
Agentes criados rodam em shells novos que podem não ter o diretório bin do pip no PATH. Um symlink em `~/bin` garante que o `agentteam` esteja sempre acessível:
...
Se `which agentteam` não retornar nada, encontre o binário manualmente:
...
O arquivo de skill ensina os agentes OpenClaw a usar o AgentTeam através de linguagem natural. Pule este passo se não estiver usando OpenClaw.
...
Agentes OpenClaw criados precisam de permissão para executar comandos `agentteam`. Sem isso, os agentes ficarão bloqueados em prompts interativos de permissão.
...
|---------|-------|-----|
| `agentteam: command not found` | Diretório bin do pip não está no PATH | Execute o Passo 3 (symlink + PATH) |
| Agentes criados não encontram o `agentteam` | Agentes rodam em shells novos sem o PATH do pip | Verifique se o symlink `~/bin/clawteam` existe e se `~/bin` está no PATH |
...
| `agentteam --version` mostra "Coming Soon" | Pacote npm usurpador instalado por engano (`a9logic`, sem relação com este projeto) | `npm uninstall -g agentteam`, depois reinstalar conforme o passo 2 |
...
Este fork torna o [OpenClaw](https://openclaw.ai) o **agente padrão**. Sem o AgentTeam, cada agente OpenClaw trabalha isoladamente. O AgentTeam o transforma em uma plataforma multi-agente.
...
| "Lance uma equipe de análise de fundo de investimento" | `agentteam launch hedge-fund` com 7 agentes |
| "Verifique o status da minha equipe de agentes" | `agentteam board show` com saída kanban |
...
Todo o estado fica em `~/.agentteam/` como arquivos JSON. Sem banco de dados, sem servidor. Escritas atômicas com travamento de arquivos `fcntl` garantem segurança contra falhas.

| Configuração | Variável de ambiente | Padrão |
|---------|---------|---------|
| Diretório de dados | `CLAWTEAM_DATA_DIR` | `~/.agentteam` |
...
**AgentTeam** — *Inteligência de enxame de agentes.*
