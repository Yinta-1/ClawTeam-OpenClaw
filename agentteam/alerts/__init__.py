...
class AlertManager:
    ...
    Example:
        ...
        manager.add_rule(AlertRule(
            ...
            condition=lambda m: m.get("agentteam.agents.active", 0) > 10,
...
def create_builtin_rules() -> dict[str, AlertRule]:
    ...
    from agentteam.metrics import get_metrics_collector
    ...
    rules["critical_agent_count"] = AlertRule(
        ...
        condition=lambda m: m.get("agentteam.agents.active", 0) > 20,
    ...
    rules["high_api_latency"] = AlertRule(
        ...
        condition=lambda m: m.get("agentteam.api.latency.p99", 0) > 1.0,
    ...
    rules["high_token_usage"] = AlertRule(
        ...
        condition=lambda m: m.get("agentteam.token.usage_rate", 0) > 100000,
        severity=AlertSeverity.WARNING,
        tags={"category": "cost"},
    ...
    rules["no_active_sessions"] = AlertRule(
        ...
        condition=lambda m: m.get("agentteam.sessions.active", 0) == 0,
    ...
    rules["high_task_queue"] = AlertRule(
        ...
        condition=lambda m: m.get("agentteam.tasks.queued", 0) > 100,
        severity=AlertSeverity.WARNING,
        tags={"category": "tasks"},
    )
...
# Additional convenience functions expected by agentteam/__init__.py
...
def check_agent_failure_rates(
    ...
    total = metrics.get_counter("agentteam.agents.total")
    failed = metrics.get_counter("agentteam.agents.failed")
...
def check_task_timeouts(
    ...
    metrics = get_metrics_collector()
    timeouts = metrics.get_counter("agentteam.tasks.timeouts")
    total = metrics.get_counter("agentteam.tasks.completed")
