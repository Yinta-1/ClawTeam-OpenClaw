...
- If you receive a new task:
  1. Acknowledge receipt: `agentteam inbox receive {team_name} --agent {agent_name}`
  2. Execute the task
  3. Report completion to leader: `agentteam inbox send {team_name} {leader_name} "Task completed: [BRIEF_SUMMARY]"`
...
- When you receive a task assignment, create a board task immediately:
  `agentteam task create {team_name} --title "[TASK_TITLE]" --status pending --owner [AGENT] --description "[DESCRIPTION]"`
- When you assign a task to a member, update the board:
  `agentteam task update {team_name} [TASK_ID] --status in_progress`
- When a member reports task completion, update the board:
  `agentteam task update {team_name} [TASK_ID] --status completed`
...
def build_agent_prompt(
    ...
    lines.extend(
        [
            ...
            f'  `agentteam inbox send {team_name} {leader_name} "Task completed: [BRIEF_SUMMARY]"',
            "- If you are blocked or any agentteam command is denied/fails, message the leader immediately with the exact error text:",
            f'  `agentteam inbox send {team_name} {leader_name} "Blocked: [EXACT_ERROR]"',
            f"- After finishing work, report your costs: `agentteam cost report {team_name} --input-tokens [N] --output-tokens [N] --cost-cents [N]`",
            f"- Before finishing, save your session: `agentteam session save {team_name} --session-id [ID]`",
            "",
            METACOGNITION_BLOCK,
            "",
        ]
