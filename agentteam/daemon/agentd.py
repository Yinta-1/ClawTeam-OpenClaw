...
运行方式：
    ...
    agentteam agentd stop   # 停止守护进程
    agentteam agentd status # 查看状态
...
架构：
┌─────────────────────────────────────────────────────────────┐
│                   agentteam CLI (Client)                     │
│  agentteam spawn / inbox send / agentd stop                 │
└─────────────────────┬───────────────────────────────────────┘
                      │ IPC (Unix Socket / TCP)
┌─────────────────────▼───────────────────────────────────────┐
...
class AgentDaemon:
    ...
    def _build_task_message(
        ...
        lines = [
            ...
            f'agentteam inbox send {team_name} leader "Task completed by {agent_name}: [BRIEF_SUMMARY]"',
            "",
            f"# Ask if should continue or exit",
            f'agentteam inbox send {team_name} leader "Task done. Should I exit or await new tasks?"',
            "```",
            "",
