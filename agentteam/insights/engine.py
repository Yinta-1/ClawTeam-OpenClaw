...
依赖：agentteam.database.DatabaseManager
"""
...
class InsightsEngine:
    ...
    def _get_conn(self) -> sqlite3.Connection:
        ...
        if self.db_path:
            ...
            # 使用默认数据库位置
            from agentteam.paths import get_data_dir

            data_dir = Path(get_data_dir())
            data_dir.mkdir(parents=True, exist_ok=True)
            db_file = data_dir / "agentteam.db"
            self.conn = sqlite3.connect(str(db_file))
    ...
    def get_token_usage_stats(self, days: int = 30, team_id: str = None) -> TokenUsageStats:
        ...
        try:
            # 尝试从数据库获取使用统计
            from agentteam.database import DatabaseManager
    ...
    def generate_report(self, days: int = 30, team_id: str = None, format: str = "json") -> Dict[str, Any]:
        ...
        report["summary_metrics"] = {
            ...
            "most_active_hour": max(activity_stats.by_hour.items(), key=lambda x: x[1])[0]
            if activity_stats.by_hour
            else None,
