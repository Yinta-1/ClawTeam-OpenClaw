"""
agentteam daemon 命令 - 与 agentd 守护进程通信
...
用法：
    ...
    agentteam daemon spawn        # 通过 daemon spawn Agent
"""
...
DATA_DIR = Path("~/.agentteam").expanduser()
...
app = typer.Typer(help="Manage the agentteam agent daemon")
...
def _send_daemon_command(command: str, args: dict = None) -> dict:
    ...
    if IS_WINDOWS:
        if not PID_FILE.exists():
            return {"ok": False, "error": "Daemon not running. Run 'agentteam daemon start' first."}
