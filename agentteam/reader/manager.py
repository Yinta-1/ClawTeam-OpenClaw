...
from agentteam.reader.types import BaseOutputReader, OutputEvent
...
class OutputReaderManager:
    ...
    def start_watching(self, session_id: str, provider_id: str, work_dir: str) -> None:
        ...
        logger.debug(
            "[OutputReaderManager] Started watching session %s for provider %s",
            session_id,
            provider_id,
        )
    ...
    def on_conversation_id_detected(self, session_id: str, conversation_id: str) -> None:
        ...
        if reader:
            ...
            logger.debug(
                "[OutputReaderManager] Bound conversation %s for session %s",
                conversation_id,
                session_id,
            )
